...
/**
 * book-cover.php — Authenticated Book Cover Image Serving Endpoint
 *
 * GET ?book_id=X  Streams the stored BLOB for the requested book.
 *                 If no cover is stored, redirects to the placeholder image.
 *
 * Authentication: any valid logged-in session (librarian, admin, or borrower).
 * Unauthenticated requests are redirected to login.php (FR-012).
 *
 * Feature: 008-book-cover-blob
 */
...
// Reuse the standard guard so suspended/deleted sessions are blocked too.
...
// Cache-Control: private prevents CDN/proxy caching (image requires auth).
